    ...
    <nav className="fixed top-0 w-full z-50 glass">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <span className="font-semibold tracking-tight text-bone-50 text-sm md:text-base">Tengnoupal Christian Church</span>
        </div>
        ...
        <div className="md:hidden flex items-center">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="right" className="bg-forest-900 border-white/10 text-bone-50 w-[300px] sm:w-[400px]">
              <SheetHeader>
                <SheetTitle className="text-bone-50 text-left">Navigation</SheetTitle>
              </SheetHeader>
              <div className="flex flex-col gap-6 mt-8">
                {navLinks.map(link => (
                  <button 
                    ...
                    className="text-left text-lg font-medium text-bone-100 hover:text-amber-accent transition-colors pb-2 border-b border-white/5"
                  >
                    {link.name}
                  </button>
                ))}
