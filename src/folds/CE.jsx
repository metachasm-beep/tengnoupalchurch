    ...
    >
      ...
      {member.img ? (
        <ImageModal 
          ...
          className={`w-20 h-20 md:w-24 md:h-24 object-cover rounded-full mb-2 md:mb-3 border-2 border-white/10 shadow-lg shrink-0 ${member.name?.includes('Seilenjam') ? 'object-[center_35%]' : member.name?.includes('Tongminthang') || member.name?.includes('Tongkhosei') ? 'object-[center_30%]' : 'object-top'}`}
        />
      ) : (
